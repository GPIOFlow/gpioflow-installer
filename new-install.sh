...
WAR_URL="https://onprempdf.com/war/gpioflow.war"
WAR_SHA256="c5305f490164707728c8a006d585d6d51af52bb17c00197d5f92b9b568320883"
# -------------------------
# DB user 
# -------------------------
DB_NAME="gpioflow"
...
require_root() {
  if [[ "${EUID}" -ne 0 ]]; then
    echo "Bitte als root ausfÃ¼hren (oder: sudo ./install.sh)"
    exit 1
  fi
}
...
if systemctl list-unit-files | grep -q '^tomcat9\.service'; then
    echo "[gpioflow] ¸ Tomcat9 Service exists. Cancel installation."
    exit 1
fi

if [ -d "/opt/tomcat" ]; then
    echo "[gpioflow] ¸ Tomcat-dir /opt/tomcat exists. Cancel installation."
    exit 1
fi


if ! id -u "${TOMCAT_USER}" >/dev/null 2>&1; then
  useradd -r -U -s /usr/sbin/nologin "${TOMCAT_USER}"
fi
...
if [[ -z "${JAVA_HOME}" ]]; then
  echo "Konnte JAVA_HOME nicht ermitteln. PrÃ¼fe Java Installation."
  exit 1
fi
...
# Generiere zufÃ¤lliges Passwort, falls noch nicht gesetzt
...
sudo chmod 640 /etc/gpioflow/db.conf
...
# ----------------------------------------------------------
# Create secure API config (api.conf)
# ----------------------------------------------------------

API_USER="apiuser"
API_PASS="$(openssl rand -base64 16)"

tee /etc/gpioflow/api.conf > /dev/null <<EOF
API_ENABLED=true
API_USER=${API_USER}
API_PASS=${API_PASS}
EOF

chown root:tomcat /etc/gpioflow/api.conf
chmod 640 /etc/gpioflow/api.conf

echo "=========================================================="
echo "GPIOFlow API credentials (shown once):"
echo "User: ${API_USER}"
echo "Pass: ${API_PASS}"
echo "=========================================================="
...
# ----------------------------------------------------------
# Create secure runtime state dir (device.id, license.json)
# ----------------------------------------------------------
mkdir -p /etc/gpioflow/state

# Tomcat (Java) darf hier lesen + schreiben
chown tomcat:tomcat /etc/gpioflow/state
chmod 700 /etc/gpioflow/state

# Root behÃ¤lt Kontrolle Ã¼ber den Config-Ordner selbst
chown root:root /etc/gpioflow
chmod 755 /etc/gpioflow
